...
export function getCommunicationInstructions(attitudeProfile, typologyProfile) {
  ...
  if (attitudeProfile && attitudeProfile.scales) {
    console.log('✅ Attitude Profile имеет scales:', attitudeProfile.scales);
    console.log('✅ Attitude Profile имеет thresholds:', attitudeProfile.thresholds);
    
    const dominantScales = Object.entries(attitudeProfile.scales)
      .filter(([scale, score]) => {
        const threshold = attitudeProfile.thresholds[scale];
        if (!threshold) {
          console.log(`❌ Нет threshold для scale ${scale}`);
          return false;
        }
        console.log(`🔍 Scale ${scale}: score=${score}, threshold.high_min=${threshold.high_min}`);
        return score >= threshold.high_min;
      })
      .sort(([,a], [,b]) => b - a)
      .slice(0, 2); // Берем 2 доминирующие шкалы

    console.log('🎯 Доминирующие шкалы:', dominantScales);
    ...
    if (dominantScales.length > 0) {
      ...
      console.log('❌ Нет доминирующих шкал');
    }
  } else {
    console.log('❌ Attitude Profile не имеет scales или thresholds');
  }

  // Добавляем инструкции по Typology профилю
  if (typologyProfile && typologyProfile.dominant && typologyProfile.dominant.length > 0) {
    console.log('✅ Typology Profile имеет dominant:', typologyProfile.dominant);
    
    // Проверяем, что это не неакцентуированная личность
    if (typologyProfile.dominant[0] === 'unaccentuated') {
      console.log('✅ Неакцентуированная личность');
      instructions.typology = {
        name: 'Неакцентуированная личность',
        description: 'Сбалансированный психологический профиль без выраженных акцентуаций',
        promise: 'Я буду общаться с вами в стандартном режиме, учитывая ваши индивидуальные особенности',
        voice_type: 'Спокойный, нейтральный',
        interaction_frequency: 'Средняя',
        positive_communication: [
          'Использовать стандартные подходы к коммуникации',
      ...
      // Обновляем общие инструкции для неакцентуированной личности
      instructions.general.key_points.push(...instructions.typology.positive_communication);
    } else {
      // Для акцентуированных типов используем новую структуру
      ...
      if (typeInstructions) {
        console.log('✅ Найдены инструкции для типа:', dominantType);
        instructions.typology = {
        ...
        console.log(`❌ Нет инструкций для типа ${dominantType}`);
      }
    }
  } else {
    console.log('❌ Typology Profile не имеет dominant или он пустой');
  }
