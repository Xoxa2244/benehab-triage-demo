...
export function computeAttitude(answersMap = {}) {
  ...
  const scales = ['severity','secondary_gain','hide_resist','work_escape','low_selfesteem','alt_med','addictions','ignore','anxiety'];
  ...
  const risk_tags = []; const comm_flags = new Set();
  scales.forEach((s) => { const raw = sums[s] || 0; const lvl = level(s, raw); profile[s] = { raw, level: lvl }; });
  if (profile.alt_med?.level === 'high') { risk_tags.push('alt_medicine_pref'); comm_flags.add('use_facts_no_pressure'); }
  if (profile.ignore?.level !== 'low') { risk_tags.push('ignore_risk'); comm_flags.add('short_clear_messages'); }
  if (profile.anxiety?.level !== 'low') { risk_tags.push('anxiety_flag'); comm_flags.add('slow_pace'); }
  if (profile.work_escape?.level === 'high') { risk_tags.push('avoidance_work'); }
  if (profile.low_selfesteem?.level !== 'low') { comm_flags.add('praise_specific'); }
  if (profile.secondary_gain?.level !== 'low') { risk_tags.push('secondary_gain_flag'); comm_flags.add('focus_outside_illness'); }
